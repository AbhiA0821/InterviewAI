...
export default function LoginPage() {
  ...
  const handleEmailSignIn = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError("Failed to sign in with specified email.");
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="w-full max-w-5xl overflow-hidden rounded-3xl border border-slate-800 bg-[#0e131f] shadow-2xl grid grid-cols-1 md:grid-cols-2">
        ...
        <div className="p-8 sm:p-12 flex flex-col justify-between space-y-8 bg-[#0b0f19]">
          <div className="space-y-6">
            ...
            {/* Universal Google Sign In Button for Any User Account */}
            <button
              type="button"
              onClick={handleGoogleSignIn}
              disabled={loading}
              className="w-full flex items-center justify-between rounded-full bg-slate-800/80 border border-slate-700 px-5 py-3.5 text-sm text-slate-200 hover:border-emerald-400/50 hover:bg-slate-800 transition-all group shadow-md active:scale-98"
            >
              <div className="flex items-center gap-3">
                ...
                <div className="text-left">
                  <div className="text-xs font-extrabold text-white group-hover:text-emerald-300 transition-colors">Sign in with Google</div>
                  <div className="text-[11px] text-slate-400">Fast Google Account Authentication</div>
                </div>
              </div>
            ...
            {error && (
              <div className="rounded-xl border border-red-500/40 bg-red-500/10 p-3 text-xs text-red-400 text-center font-semibold">
                {error}
              </div>
            )}
          ...
          <div className="text-center text-xs text-slate-400 pt-4">
            ...
            <span onClick={handleGoogleSignIn} className="font-bold text-white cursor-pointer underline hover:text-emerald-400 transition-colors">
              Sign up
            </span>
          </div>
        </div>
