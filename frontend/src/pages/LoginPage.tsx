...
import { Sparkles, Phone, ShieldCheck, CheckCircle2, ArrowRight, Bot, Lock, Smartphone } from "lucide-react";
      ...
      <div className="relative w-full max-w-4xl overflow-hidden rounded-3xl border border-slate-800/80 bg-slate-950/90 backdrop-blur-2xl shadow-[0_0_50px_-12px_rgba(16,185,129,0.25)] grid grid-cols-1 md:grid-cols-2">
        ...
        <div className="relative z-10 p-8 sm:p-10 flex flex-col justify-between space-y-6 bg-slate-900/60 backdrop-blur-md">
          <div className="space-y-6">
            ...
            {/* ANIMATED MODE SELECTOR SWITCH TABS */}
            <div className="grid grid-cols-2 gap-2 p-1.5 rounded-2xl bg-slate-950/90 border border-slate-800 shadow-inner">
              <button
                type="button"
                ...
                {!otpSent ? (
                  <form onSubmit={handleSendPhoneOtp} className="space-y-3">
                    <div className="relative group">
                      <input
                        ...
                        className="w-full rounded-2xl border-2 border-slate-700/80 bg-slate-950 px-4 py-3.5 text-sm text-white placeholder:text-slate-500 focus:border-emerald-400 focus:outline-none focus:ring-2 focus:ring-emerald-400/20 font-bold transition-all"
                      />
                      <Phone className="absolute right-4 top-4 h-4 w-4 text-slate-400 group-hover:text-emerald-400 transition-colors" />
                    </div>

                    <button
                      type="submit"
                      disabled={loading || !phone.trim()}
                      className="w-full rounded-2xl bg-gradient-to-r from-emerald-500 via-teal-500 to-emerald-600 hover:from-emerald-400 hover:to-teal-400 py-3.5 text-xs font-black text-white transition-all duration-300 shadow-xl shadow-emerald-500/25 hover:shadow-emerald-500/40 disabled:opacity-40 flex items-center justify-center gap-2 active:scale-95"
                    >
                      <span>{loading ? "Sending SMS Code..." : "Send Mobile SMS OTP"}</span>
                  ...
                  <form onSubmit={handleVerifyPhoneOtp} className="space-y-3">
                    ...
                    <input
                      ...
                      placeholder="123456"
                      className="w-full rounded-2xl border-2 border-emerald-500/80 bg-slate-950 px-4 py-3.5 text-center text-xl font-mono font-black tracking-widest text-emerald-300 focus:outline-none focus:ring-2 focus:ring-emerald-400 shadow-inner"
                    />
                    ...
                    <div className="flex gap-2">
                      <button
                        ...
                        className="w-1/3 rounded-2xl border border-slate-700 bg-slate-800 py-3 text-xs font-extrabold text-slate-300 hover:bg-slate-700 transition-all"
                      >
                        Change Number
                      </button>
                      <button
                        ...
                        className="w-2/3 rounded-2xl bg-gradient-to-r from-emerald-500 to-teal-500 hover:from-emerald-400 hover:to-teal-400 py-3 text-xs font-black text-white transition-all shadow-lg shadow-emerald-500/25 disabled:opacity-40 flex items-center justify-center gap-2 active:scale-95"
                      >
          ...
          <div className="text-center text-[11px] text-slate-500 font-medium flex items-center justify-center gap-1.5">
            ...
            <span>End-to-End Encrypted Firebase Verification</span>
          </div>
        </div>
